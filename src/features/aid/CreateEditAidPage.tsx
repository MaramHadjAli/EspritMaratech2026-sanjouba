...
const CreateEditAidPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (aidType) payload.unit = aidType;
    try {
      setSubmitting(true)
      // OFFLINE MODE
      if (isOffline) {
        await queueAction({
          type: isEditing ? 'UPDATE_AID' : 'CREATE_AID',
          endpoint: isEditing ? `/aids/${id}` : '/aids',
          method: isEditing ? 'PUT' : 'POST',
          payload,
          createdAt: new Date().toISOString(),
          retries: 0,
          status: 'pending',
          label: `${lang === 'fr' ? 'Aide' : 'Aid'}: ${aidType} - ${name}`,
        })
      ...
      if (!navigator.onLine && queueAction) {
        await queueAction({
          type: isEditing ? 'UPDATE_AID' : 'CREATE_AID',
          endpoint: isEditing ? `/aids/${id}` : '/aids',
          method: isEditing ? 'PUT' : 'POST',
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            {/* Get Deposits Button (separate) */}
            <div className="mt-6">
              <button
                type="button"
                disabled={recommendLoading}
                className="w-full px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                onClick={async () => {
                  setRecommendLoading(true)
                  setRecommendError('')
                  try {
                ...
                <div className="mb-4 flex gap-2 overflow-x-auto">
                  {recommendedDeposits.map(deposit => {
                    ...
                    return (
                      <div
                        key={deposit.id}
                        className={`relative flex flex-col items-center p-2 rounded-md shadow border transition-all cursor-pointer ${selectedDepositId === deposit.id ? 'border-primary-500 bg-primary-50 dark:bg-primary-900' : 'border-gray-200 bg-white dark:bg-gray-800'}`}
                        style={{ minWidth: 120, maxWidth: 140, zIndex: 10 }}
                        onClick={() => setSelectedDepositId(deposit.id)}
                      >
                        <img src={deposit.containerImageUrl} alt={deposit.name} className="w-16 h-10 object-cover rounded mb-1" />
                        ...
                        <div className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-0.5 truncate">{deposit.name}</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400 mb-0.5 truncate">{deposit.city} ({deposit.region})</div>
            ...
            <div className="mt-6">
              <button
                ...
                className="w-full px-4 py-2 text-sm font-semibold text-white bg-primary-600 rounded-lg hover:bg-primary-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {submitting
                  ? lang === 'fr'
                    ? 'Envoi...'
